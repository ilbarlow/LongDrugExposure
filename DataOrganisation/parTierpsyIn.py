...
def ChunkTraj (timeseriesDF, chunkSize, conditions, name):
    ...
    Output:
        trajectoryChunks - a dictionary containing the timeseries data chunked 
...
TrajDir = os.path.join(os.path.dirname(directoryB), '5minChunkedTrajectories')
try:
    os.mkdir(TrajDir)
...
for f in fut:
    savedir = os.path.join(TrajDir, '_'.join(f.result()[2].split('.')) + '_Chunkedtrajectories.hdf5')
#    try:
#        os.mkdir(savedir)
